...
static void
handleRange (unsigned long pending, const unsigned int base)
{
  while (pending)
  {
...
void
irqRegister (const unsigned int irq, FN_INTERRUPT_HANDLER pfnHandler, void *pParam)
{
...
void
irqEnable (const unsigned int irq)
{
...
void
irqDisable (const unsigned int irq)
{
  unsigned long mask = 1UL << (irq % 32);
